package ltd.hayl.capacitor.plugins.pushnotifications;
    ...
    public void listChannels(PluginCall call) {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            ...
            for (NotificationChannel notificationChannel : notificationChannels) {
                ...
                channel.put(CHANNEL_LIGHT_COLOR,
                        String.format("#%06X", (0xFFFFFF & notificationChannel.getLightColor())));
                Logger.debug(Logger.tags("NotificationChannel"),
    ...
    public void createForegroundNotificationChannel() {
        ...
        if ((presentation != null) && (androidForegroundChannelId.equals("default"))) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                String description = config.getString("androidForegroundChannelDescription",
                        "Push notifications in foreground");
                int importance = NotificationManager.IMPORTANCE_HIGH;
                NotificationChannel channel = new NotificationChannel(FOREGROUND_NOTIFICATION_CHANNEL_ID, name,
                        importance);
                ...
                if (Arrays.asList(presentation).contains("sound")) {
                    ...
                    if (sound != null) {
                        soundUri = Uri.parse(
                                ContentResolver.SCHEME_ANDROID_RESOURCE + "://" + context.getPackageName() + "/raw/"
                                        + sound);
                    } else {
